...
const purchaseConfirmationEmail = async (
  ...
  console.log("product id en purchaseconfirmation", productId);
  const ruta = `/products/product/?id=${productId}`;
  ...
  const customHtml = (link) => {
    ...
    <p class="t63" style="margin:0;Margin:0;font-family:BlinkMacSystemFont,Segoe UI,Helvetica Neue,Arial,sans-serif,'Albert Sans';line-height:21px;font-weight:400;font-style:normal;font-size:16px;text-decoration:none;text-transform:none;letter-spacing:-0.64px;direction:ltr;color:#333333;text-align:left;mso-line-height-rule:exactly;mso-text-raise:2px;">No olvides valorar el producto una vez que hayas recibido el mismo. Essperamos que disfrutes de tu compra</p></td>
    </tr></table>
  ...
  await sendMail(
    email,
    "Tu compra ha sido aceptada con éxito",
    link,
    customHtml
  );
};
